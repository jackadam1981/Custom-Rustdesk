...
| 项 | 状态 |
|----|------|
| `CUSTOM_VERIFIED_PATCH_UP_TO` | **`""`**（UI 针重编号后待 bump；全量见 [`patch-registry.md`](./patch-registry.md)） |
| **Sciter UI** | patch-lab 已验 — S10–S15（logo/名/slogan/powered/about/菜单） |
| **Flutter UI** | patch-lab 已验 — F10–F14（同上控件对齐） |
| 本地静态 | `bash scripts/patch-lab/step-verify-all.sh --profile baixin`（22 针） |

### Sciter 验收记录（已固化）

| Release | Run | 验收项 |
|---------|-----|--------|
| `baixin-menu-20260627-20260627-140756` | [28291552327](https://github.com/jackadam1981/Custom-Rustdesk/actions/runs/28291552327) | 首页品牌、Powered by、关于 studio-about、三点菜单（后改 **单列+滚动**，见 S15） |
...
2. **F13** — `powered-by.sh` 用 Python + `getConnectionPageTitle` 锚点注入，兼容上游 **CRLF**（Windows patch-lab clone）。
...
`R01 R03 B01 B02 I01 F02 F10 F11 F12 F13 F14 S10 S11 S12 S13 S14 S15 P01 P02 P03 P04`

详见 [`patch-registry.md`](./patch-registry.md)。脚本目录：`.github/workflows/scripts/patches/`（`source-patcher.sh` 为入口）。
