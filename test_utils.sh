...
check_dependencies() {
    ...
    if ! command -v jq &> /dev/null; then
        log_error "jq 未安装"
        log_info "请安装 jq: sudo apt install jq"
        exit 1
    fi
...
get_workflow_info() {
    ...
    if [ -z "$target_workflow" ]; then
        log_error "未找到目标工作流: CustomBuildRustdesk.yml"
        exit 1
    fi
...
trigger_workflow_dispatch() {
    ...
    log_info "使用 workflow_dispatch 触发工作流..."
    ...
    local trigger_result=$(gh workflow run "$workflow_id" \
        ...
        --field enable_debug="$enable_debug" \
        --json id,url \
        2>/dev/null || echo "")
...
monitor_workflow_run() {
    ...
    local check_interval=30   # 30秒
    local elapsed_time=0
    
    log_info "开始监控工作流运行 (ID: $run_id)"
    log_info "最大等待时间: ${max_wait_time}秒"
    ...
    while [ $elapsed_time -lt $max_wait_time ]; do
        ...
        if [ -n "$run_status" ]; then
            ...
            local created_at=$(echo "$run_status" | jq -r '.createdAt')
            local updated_at=$(echo "$run_status" | jq -r '.updatedAt')
            local run_url=$(echo "$run_status" | jq -r '.url')
            
            log_info "状态: $status, 结论: $conclusion"
            log_info "创建时间: $created_at"
            log_info "更新时间: $updated_at"
            log_info "运行URL: $run_url"
...
get_workflow_logs() {
    ...
    if [ -d "$log_dir" ] && [ "$(ls -A "$log_dir" 2>/dev/null)" ]; then
        ...
        log_info "日志文件列表:"
        find "$log_dir" -type f -name "*.txt" | head -10 | while read -r file; do
            log_info "  - $file"
        done
    else
        log_warning "未找到日志文件"
